...
// Button (Moved to A1 to prevent clashing with I2C SCL on A5)
...
void loop() {
  ...
  if (lastButtonState == HIGH && currentButtonState == LOW) {
    ...
    if (satellites > 0) {
      ...
      // 2. Format Data into a single String (e.g., "43.589001N, -79.644104W, 156.20")
      String gpsStr = String(lat.latitudeDegree, 6) + String((char)lat.latDirection) + ", " + 
                      String(lon.lonitudeDegree, 6) + String((char)lon.lonDirection) + ", " + 
                      String(alt, 2);
      ...
      if (c.result(ok) && ok) {
        Monitor.print("=> Saved to Linux JSON: ");
        Monitor.println(gpsStr);

        // Verify immediate load (Assumes Python's verify function returns the whole string)
        String verifyStr = "";
